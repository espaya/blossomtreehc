            ...
            <div class="container">
                <div class="content-main lg:px-[60px] md:px-4 flex gap-y-8 max-md:flex-col w-full">
                    ...
                    <div class="text-content w-full">
                        ...
                        @endif
                            <form action="{{ route('update.account.details') }}" method="post" enctype="multipart/form-data" class="">
                                @csrf
                                @method('POST')
                                ...
                                <div class="grid sm:grid-cols-2 gap-4 gap-y-5">
                                    ...
                                    <div class="phone-number">
                                        <input name="phone" value="{{ $user->phone }}" class="border-line px-4 pt-3 pb-3 w-full rounded-lg" id="phoneNumber" type="text">
                                        @error('phone')
                                        <p style="color:red"> {{ $message }} </p>
                                        @enderror
                                    </div>
                                ...
                                <div class="pass">
                                    <input name="password" class="border-line px-4 pt-3 pb-3 w-full rounded-lg" id="password" type="password" placeholder="Old Password (leave blank to remain unchange)">
                                    @error('password')
                                        <p style="color:red"> {{ $message }} </p>
                                        @enderror
                                ...
                                <div class="new-pass mt-5">
                                    <input name="new_password" class="border-line px-4 pt-3 pb-3 w-full rounded-lg" id="newPassword" type="password" placeholder="New Password (leave blank to remain unchange)">
                                    @error('new_password')
                                        <p style="color:red"> {{ $message }} </p>
                                        @enderror
                                </div>
                                <div class="confirm-pass mt-5">
                                    <input name="password_confirmation" class="border-line px-4 pt-3 pb-3 w-full rounded-lg" id="confirmPassword" type="password" placeholder="Confirm Password (leave blank to remain unchange)">
